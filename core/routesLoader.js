...
async function regenerateSwaggerSpec() {
    try {
        ...
        const swaggerModule = await import(`../config/swagger.js?update=${Date.now()}`);
        const newSwaggerSpec = swaggerModule.default;
        console.log('📚 Swagger documentation regenerated');
        return newSwaggerSpec;
    } catch (error) {
        console.error('❌ Failed to regenerate Swagger spec:', error);
        return null;
    }
}
                    ...
                    files.forEach(async (file) => {
                        if (file.endsWith('.controller.js')) {
                            ...
                            if (controller && typeof controller === 'function') {
                                console.log(`✅ Mounting [${baseApiPath}/${version}/${folder}] -> [${version}/${folder}/${file}]`);
                                router.use(controller);
                            } else {
                                console.warn(`⚠️ Controller ${version}/${folder}/${file} is invalid.`);
                            }
                        }
                    });
 ...
 */
function setupDynamicRoutes(app, baseApiPath = '/api', controllersPath = path.join(__dirname, '..', 'controllers')) {
    // Load initial routes
    ...
    try {
        swaggerUi = app._router.stack.find(layer =>
            layer.route && layer.route.path === '/api-docs');
    } catch (error) {
        console.warn('⚠️ Could not find Swagger UI route for dynamic updates');
    }
    ...
    const handleControllerChange = async (filePath) => {
        if (filePath.endsWith('.controller.js')) {
            console.log(`[CONTROLLER CHANGED] ${filePath}`);
            ...
            if (updatedSpec && swaggerUi) {
                ...
                console.log('📚 Swagger UI will use updated spec on next request');
            }
    ...
    watcher.on('unlink', (filePath) => {
        if (filePath.endsWith('.controller.js')) {
            console.log(`[CONTROLLER REMOVED] ${filePath}`);
